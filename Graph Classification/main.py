...
parser.add_argument("--batch_size", type=int, default=48, help="input batch size for training (default: 16)")
parser.add_argument("--learning_rate", type=float, default=0.0023180472987803877, help="learning rate for optimizer")
parser.add_argument("--epochs", type=int, default=10, help="epochs for training")
parser.add_argument("--federated_rounds", type=int, default=10, help="federated rounds performed")
...
for round_id in range(1,args.federated_rounds + 1):
    for i in range(args.parameterC):
        subprocess.run(["./initialize", str(i + 1)])
    ...
    for MyClient in Client_list:
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            if early_stopping_callback.early_stop:
                break
            ModelWriter(MyClient.model.state_dict(), MyClient.id)
            subprocess.run(["./encrypt", str(MyClient.id)])
        #Keep all epochs best
    ...
    server_acc, f1, precision, recall = perform_federated_avg(server_model, Client_list,round_id,test_loader, args, shapes)
